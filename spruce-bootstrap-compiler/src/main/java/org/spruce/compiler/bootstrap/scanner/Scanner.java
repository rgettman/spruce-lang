...
//    }
    ...
    /**
     * Reads and returns "other" tokens, starting with punctuation.
     * @return The <code>Token</code> that was read.
     */
    private Token readOtherToken() {
        return switch(peek()) {
            ...
            default -> {
                char unrecognized = read();
                yield createToken(TokenType.UNKNOWN, String.valueOf(unrecognized),
                        getLocation(), "Unrecognized character: '" + unrecognized + "'.");
            }
        };
    }
    ...
    private Token readStartingWithExclamation() {
        ...
        if (peek() == '=') {
            read();
            return createToken(TokenType.EXCLAMATION_EQUAL, "!=");
        }
    ...
    /**
     * Scans "traditional" comment start/comment/end
    ...
    private Token readStartingWithSlash() {
        ...
        return switch (peek()) {
            ...
            default -> createToken(TokenType.UNKNOWN, "/");
        };
    }
    ...
    private Token readStartingWithAmpersand() {
        ...
        if (peek() == '&') {
            read();
            return createToken(TokenType.DOUBLE_AMPERSAND, "&&");
        }
        else {
    ...
    private Token readStartingWithPipe() {
        ...
        if (peek() == '|') {
            read();
            return createToken(TokenType.DOUBLE_PIPE, "||");
        }
        else {
            return createToken(TokenType.UNKNOWN, "|");
        }
