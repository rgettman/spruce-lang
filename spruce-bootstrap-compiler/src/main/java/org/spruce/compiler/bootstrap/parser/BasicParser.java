...
public class BasicParser {
     ...
     * @param separatorToken The accepted <code>TokenType</code>
    ...
    protected <C extends Node, L extends ASTListNode<C>> L parseList(Predicate<Token> isOnInitialToken,
              String initialErrorMessage, TokenType separatorToken, Supplier<C> childParser,
              BiFunction<Location, List<C>, L> nodeSupplier) {
        return parseList(isOnInitialToken, initialErrorMessage, separatorToken, childParser,
                nodeSupplier, true);
    }
    ...
    protected <C extends Node, L extends ASTListNode<C>> L parseList(Predicate<Token> isOnInitialToken,
        ...
        while (isCurr(separatorToken) && (isOnInitialToken.test(next()))) {
            accept(separatorToken);
            children.add(childParser.get());
        }
    ...
    protected <C extends Node, L extends ASTListNode<C>> L parseMultiple(Predicate<Token> isOnInitialToken,
              ...
              BiFunction<Location, List<C>, L> nodeSupplier) {
        return parseMultiple(isOnInitialToken, initialErrorMessage, childParser,
                nodeSupplier, true);
    }
    ...
    protected static boolean isValueExpression(Token t) {
        return test(t, Arrays.asList(EXCLAMATION, MINUS)) || isPrimary(t);
    }
    ...
    protected static boolean isPrimary(Token t) {
        return isLiteral(t) || test(t, Arrays.asList(IDENTIFIER, SELF, OPEN_PARENTHESIS, NEW, SUPER, OPEN_BRACE));
    }
